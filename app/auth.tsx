...
export default function Auth(){
 ...
 return <Screen><View style={{flex:1,justifyContent:'center'}}><View style={{alignItems:'center',marginBottom:14}}><SmartVibeLogo width={310} height={93}/></View><Text style={{fontSize:12,fontWeight:'900',color:C.cyan,letterSpacing:1.5,textAlign:'center'}}>SECURE CLIENT ACCESS</Text><Text style={{fontSize:34,lineHeight:40,fontWeight:'900',color:C.ink,marginTop:8,textAlign:'center'}}>{register?'Create account':'Welcome back'}</Text><Text style={{color:C.muted,fontSize:15,lineHeight:22,marginTop:8,textAlign:'center'}}>{register?'Create your SmartVibe trading account.':'Sign in to continue to your SmartVibe trading workspace.'}</Text><Text style={{textAlign:'center',color:C.muted,fontSize:13,marginVertical:10}}>Email authentication with 6-digit verification</Text>{register?<Card><Text style={{fontWeight:'900',color:C.ink}}>Username</Text><TextInput value={username} onChangeText={setUsername} autoCapitalize="none" autoCorrect={false} placeholder="Choose a username" placeholderTextColor={C.muted} style={input}/></Card>:null}<Card><Text style={{fontWeight:'900',color:C.ink}}>Email</Text><TextInput value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" autoCorrect={false} placeholder="you@example.com" placeholderTextColor={C.muted} style={input}/><Text style={{fontWeight:'900',marginTop:14,color:C.ink}}>Password</Text><TextInput value={password} onChangeText={setPassword} secureTextEntry autoCapitalize="none" autoCorrect={false} placeholder="Minimum 8 characters" placeholderTextColor={C.muted} style={input}/>{error?<Text style={{color:C.red,marginTop:10,lineHeight:20}}>{error}</Text>:null}{notice?<Text style={{color:C.green,marginTop:10,lineHeight:20}}>{notice}</Text>:null}</Card><Button title={busy?'Please wait…':register?'Create account':'Sign in'} onPress={submit} disabled={busy}/>{!register?<Button title="Forgot password?" variant="secondary" onPress={()=>{setResetMode(true);setError('');setNotice('')}} disabled={busy}/>:null}<Button title={register?'I already have an account':'Create a new account'} variant="secondary" onPress={()=>{setError('');setNotice('');setRegister(!register)}} disabled={busy}/></View></Screen>
}
