...
export default function AutoTrade() {
  ...
  const load = useCallback(async () => {
    ...
    } catch (e: any) { setSignals([]); setError(e?.message ?? 'Unable to load live SmartVibe strategy signals.'); }
  ...
  return <Screen><ScrollView showsVerticalScrollIndicator={false}>
    <Header title="SmartVibe Strategy" subtitle="Live analysis using the SmartVibe Trading Strategy workspace." right={<Badge label="LIVE" tone="positive" />} />
    <Card elevated><Text style={{ fontWeight: '900', fontSize: 18 }}>Main strategy workspace</Text><Text style={{ color: C.muted, marginTop: 5, lineHeight: 20 }}>SmartVibe Trading Strategy: establish Daily/4H direction, confirm the A-high or V-low liquidity break, then validate an FVG or Order Block entry and nearest liquidity target.</Text>
    ...
    {loading ? <LoadingState label="Requesting live SmartVibe analysis…" /> : null}
    ...
    {!loading && !error && signals.length === 0 ? <Card><Badge label="WAIT" tone="neutral" /><Text style={{ fontWeight: '900', fontSize: 20, marginTop: 8 }}>No qualifying setup</Text><Text style={{ color: C.muted, marginTop: 5, lineHeight: 20 }}>No live consensus signal currently meets the selected conditions.</Text></Card> : null}
