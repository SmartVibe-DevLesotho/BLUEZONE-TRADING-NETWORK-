...
export default function AutoTrade() {
  ...
  const load = useCallback(async () => { setError(''); setLoading(true); try {
    ...
    for (const signal of next) {
      const { data: latest } = await supabase.from('signals').select('id,generated_at').eq('symbol',signal.symbol).eq('direction',signal.direction).order('generated_at',{ascending:false}).limit(1).maybeSingle();
      if (latest) {
        await supabase.from('user_signal_history').insert({ signal_id: latest.id, action:'VIEWED' });
        const { data: usage } = await supabase.from('smartvibe_signal_usage').select('id,status').eq('signal_id',latest.id).maybeSingle();
        if (usage?.status === 'PENDING') ids[`${signal.symbol}-${signal.direction}-${signal.score}`] = usage.id;
      }
    }
    setUsageIds(ids);
  ...
  async function classify(signal: ConsensusSignal, used: boolean) { const key=`${signal.symbol}-${signal.direction}-${signal.score}`,id=usageIds[key]; if(!id)return; setUsageBusy(key); try { const {error:e}=await supabase.rpc('approve_smartvibe_signal_usage',{p_usage_id:id,p_used:used,p_reason:used?'Client confirmed signal was used':'Client confirmed signal was not used'}); if(e)throw e; setUsageIds(x=>{const y={...x};delete y[key];return y;}); await load(); } catch(e:any) { setError(e?.message??'Unable to classify signal usage.'); } finally { setUsageBusy(null); } }
