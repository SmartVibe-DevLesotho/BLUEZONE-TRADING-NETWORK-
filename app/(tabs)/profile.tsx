...
export default function Profile() {
  ...
  useEffect(()=>{(async()=>{const [{data},token]=await Promise.all([supabase?.auth.getUser() ?? Promise.resolve({data:{user:null}}),SecureStore.getItemAsync('bluezone_license_token')]);setEmail(data.user?.email??null);if(token)setLicense('Activated');setLoading(false);})();},[]);
