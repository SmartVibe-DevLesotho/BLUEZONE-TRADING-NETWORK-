  ...
  return <View style={{marginTop:14,borderRadius:14,overflow:'hidden',backgroundColor:'#07101d',borderWidth:1,borderColor:C.border}}>
    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
      <Svg width={width} height={height}>
        ...
        {indicators.htfCandles.map((c,i)=>{const l=Math.max(0,c.left-visibleIndexOffset),r=Math.min(candles.length-1,c.right-visibleIndexOffset);if(r<0||l>=candles.length)return null;const cx=x((l+r)/2),up=c.close>=c.open;return <View key={`htf-${i}`}>{/* placeholder node intentionally avoided */}<Line x1={cx} y1={y(c.high)} x2={cx} y2={y(c.low)} stroke={up?'#38bdf8':'#fb7185'} strokeWidth="1"/><Rect x={cx-bodyWidth/2} y={Math.min(y(c.open),y(c.close))} width={bodyWidth} height={Math.max(2,Math.abs(y(c.close)-y(c.open)))} fill="none" stroke={up?'#38bdf8':'#fb7185'} strokeWidth="1.2"/></View>})}
        ...
        {candles.map((c,i)=>{const up=c.close>=c.open,cx=x(i),top=y(Math.max(c.open,c.close)),bottom=y(Math.min(c.open,c.close));return <View key={`c-${i}`}><Line x1={cx} y1={y(c.high)} x2={cx} y2={y(c.low)} stroke={up?'#22c55e':'#ef4444'} strokeWidth="1.5"/><Rect x={cx-bodyWidth/2} y={Math.min(top,bottom)} width={bodyWidth} height={Math.max(2,Math.abs(bottom-top))} fill={up?'#22c55e':'#ef4444'}/></View>})}
        {indicators.markers.map((m,i)=>{const ix=m.index-visibleIndexOffset;if(ix<0||ix>=candles.length)return null;const cx=x(ix),cy=y(m.price);const buy=m.dir==='buy';return <View key={`m-${i}`}><Polygon points={buy?`${cx},${cy+16} ${cx-7},${cy+4} ${cx+7},${cy+4}`:`${cx},${cy-16} ${cx-7},${cy-4} ${cx+7},${cy-4}`} fill={buy?'#22c55e':'#ef4444'}/><SvgText x={cx+9} y={cy+(buy?20:-10)} fill={buy?'#86efac':'#fca5a5'} fontSize="10">{m.label}</SvgText></View>})}
...
function Polyline(props:any){ const R=require('react-native-svg'); return <R.Polyline {...props}/>; }
