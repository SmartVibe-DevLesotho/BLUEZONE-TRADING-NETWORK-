...
// Kept under the existing function name for app compatibility; the server endpoint is now the canonical SmartVibe engine.
export const getConsensusSignals = (input: {symbols: string[]; threshold: number; session: string; style: string}) =>
  invokeFunction<{ok?: boolean; signals: ConsensusSignal[]}>('consensus-signals', input);
