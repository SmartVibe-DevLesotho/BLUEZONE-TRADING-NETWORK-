...
export type SmartVibeBar = { t: number; o: number; h: number; l: number; c: number; v?: number };
export type SmartVibeSessionWindow = { session: SmartVibeSession; phase: SmartVibePhase; startHourSast: number; endHourSast: number };
...
export type SmartVibeMechanicId =
  | 'market-structure' | 'liquidity' | 'range-location' | 'trend-transition' | 'volatility'
  | 'crt-session-cycle' | 'time-window-confluence' | 'order-block' | 'fair-value-gap'
  | 'premium-discount' | 'displacement' | 'support-resistance' | 'candlestick-price-action'
  | 'chart-patterns' | 'ema-alignment' | 'momentum' | 'volume-context' | 'news-risk'
  | 'session-context' | 'data-quality';
...
export type SmartVibeCandlePattern =
  ...
  | 'INVERTED_HAMMER' | 'HANGING_MAN' | 'PIERCING' | 'DARK_CLOUD_COVER'
...
export function detectSmartVibeCandlesticks(bars: SmartVibeBar[]): SmartVibeCandlePattern[] {
  ...
  if (lower(c) >= cBody * 2 && upper(c) <= cBody * 0.75) out.push(bullish(c) ? 'HAMMER' : 'HANGING_MAN');
  if (upper(c) >= cBody * 2 && lower(c) <= cBody * 0.75) out.push(bullish(c) ? 'INVERTED_HAMMER' : 'SHOOTING_STAR');
  ...
  if (bullish(a) && bullish(b) && bullish(c) && b.c > a.c && c.c > b.c) out.push('THREE_WHITE_SOLDIERS');
  if (bearish(a) && bearish(b) && bearish(c) && b.c < a.c && c.c < b.c) out.push('THREE_BLACK_CROWS');
  // Harami: the current candle body must be contained inside the prior body.
  if (bearish(b) && bullish(c) && c.o >= b.c && c.c <= b.o) out.push('BULLISH_HARAMI');
  if (bullish(b) && bearish(c) && c.o <= b.c && c.c >= b.o) out.push('BEARISH_HARAMI');
  return [...new Set(out)];
}
...
export type SmartVibeChartPattern =
  ...
  | 'FALLING_WEDGE' | 'BULL_FLAG' | 'BEAR_FLAG' | 'BULL_PENNANT' | 'BEAR_PENNANT'
...
export function detectSmartVibeChartPatterns(bars: SmartVibeBar[]): SmartVibeChartPattern[] {
  ...
  const tolerance = Math.max((Math.max(...highs) - Math.min(...lows)) * 0.015, Number.EPSILON);
