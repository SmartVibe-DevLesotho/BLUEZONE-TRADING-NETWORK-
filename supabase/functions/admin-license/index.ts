...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(body.action==='list'){
      const {data:licenses,error}=await supabase.from('licenses').select('id,label,active,expires_at,max_activations,activation_count,issued_at,revoked_at').eq('issued_by',data.user.id).order('issued_at',{ascending:false}).limit(100);
    ...
    if(body.action==='revoke'){
      ...
      const {data:revoked,error:revokeError}=await supabase.from('licenses').update({active:false,revoked_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq('id',id).eq('issued_by',data.user.id).select('id').maybeSingle();
    ...
    const durationDays=Math.min(365,Math.max(1,Math.floor(Number(body.durationDays)||30)));
    const token=`SVTN-${randomHex()}`;
    const tokenHash=await sha256(token);
    const expiresAt=new Date(Date.now()+durationDays*86400000).toISOString();
    const {data:license,error:insertError}=await supabase.from('licenses').insert({token_hash:tokenHash,active:true,expires_at:expiresAt,max_activations:1,activation_count:0,issued_by:data.user.id,label:typeof body.label==='string'?body.label.trim().slice(0,120):null,issued_at:new Date().toISOString(),updated_at:new Date().toISOString()}).select('id,expires_at,max_activations').single();
    if(insertError)throw insertError;
    return json({token,expiresAt,maxActivations:1,license});
